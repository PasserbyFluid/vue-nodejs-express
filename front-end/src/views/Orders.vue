    ...
    <el-main>
      <el-card>
        ...
        <el-table :data="orders" empty-text="暂无订单" style="width: 100%">
          ...
          <el-table-column v-if="user && user.status == '1'" prop="userName" label="用户名" width="200"/>
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <el-button type="primary" icon="el-icon-view" circle @click="viewOrder(scope.row)"
                size="small"></el-button>
              <el-button type="success" icon="el-icon-wallet" circle @click="goToPay(scope.row)" size="small"
                v-if="scope.row.stateText === '待处理'"></el-button>
              <el-button type="danger" icon="el-icon-delete" circle @click="cancelOrder(scope.row)" size="small"
                v-if="scope.row.stateText === '待处理'"></el-button>
            </template>
          </el-table-column>
        </el-table>
  ...
  methods: {
    ...
    cancelOrder (order) {
      this.$confirm('确认取消该订单吗？', '提示', {
        ...
        api.post('/order/delete',{orderId:order.orderId}).then(response => {
