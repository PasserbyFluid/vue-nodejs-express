...
router.post('/login', async function (req, res) {
  ...
  let sql = "select user_id userId,user_name userName,password,telephone,status from user where user_name=?"
...
router.get('/menu/list', async function(req, res) {
  ...
  let resResult = {'code':200,'reason':'成功'};
  let sql = "select menu_id menuId,menu_name menuName,menu_role menuRole from menu"
...
router.get('/type/list', async function (req, res) {
  let param = req.query || req.params;  
  let resResult = {'code':200,'reason':'成功'};
  let sql = "select type_id typeId,type_name typeName,type_remark typeRemark,type_num typeNum from type"
  let result = await db.execQuery(sql, [])
  resResult.code = 200;
  resResult.reason = "成功";
  resResult.result = result
...
router.get('/product/list', async function (req, res) {
  let param = req.query || req.params;  
  let resResult = {'code':200,'reason':'成功'};
  let sql = "SELECT product_id AS productId,product_no AS productNo,product_name AS productName, out_price AS outPrice, CONCAT(\"http://localhost:3000/getFile/\",product_url) AS productUrl,remark,rating,ratingCount FROM product"
  let result = await db.execQuery(sql, [])
  resResult.code = 200;
  resResult.reason = "成功";
  resResult.result = result
...
router.post('/product_comments/add', async function (req, res) {
  ...
  try {
    // 添加评论
    await db.execQuery(sql, [productId, commentsContent, rating || null, userId, state]);
...
router.post('/shopping_cart/add', async function (req, res) {
  ...
  try {
    ...
    if (existResult.length > 0) {// 如果存在，更新数量和价格
...
router.post('/order/add', async function (req, res) {
  ...
  let totalNum = 0;
  let remark = ""
  let detailSql = "INSERT INTO order_detail ( order_no, product_id, product_num,product_name, out_price, subtotal,user_id ) VALUES "
  for (let i = 0; i < proudctList.length; i++) {
    let item = proudctList[i];
    total += item.outPrice * item.productNum;
    totalNum += item.productNum;
    remark += item.productName + "|";
    detailSql+=`('${orderNo}',${item.productId},${item.productNum},'${item.productName}',${item.outPrice},${(item.outPrice*item.productNum).toFixed(2)},${userId}),`
  }
...
router.get('/movie/isFavorite', async function (req, res) {
  ...
  let sql = "SELECT  count(*) count FROM collect where movie_id=? and user_id=? "
...
router.get('/movie/detail', async function (req, res) {
  ...
  let selectCommonSql = "SELECT * from movie_comments where movie_id = ? and `state` = 2 order by comments_time desc"
...
router.post('/movie/add', async function (req, res) {
  ...
  let sql = "INSERT INTO movie (movie_name, type_id, director, starring, year, remark, img_url) VALUES (?, ?, ?, ?, ?, ?, ?)";
  
  try {
    await db.execQuery(sql, [movieName, typeId, director, starring, year, remark, imgUrl]);
  } catch (e) {
...
router.post('/movie_comments/add', async function (req, res) {
  ...
  try {
    // 添加评论
    await db.execQuery(sql, [movieId, commentsContent, score || null, userId, state]);
...
router.post('/user/update', async function (req, res) {
  ...
  // 如果更新用户名，需要检查是否已存在
  if (userName) {
    let checkSql = "SELECT COUNT(*) as count FROM user WHERE user_name = ? AND user_id != ?";
